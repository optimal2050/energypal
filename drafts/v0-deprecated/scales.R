...
scale_color_energy <- function(palette = NULL,
  ...
  # New hierarchy-centric palette acquisition: if palette corresponds to legacy name, fallback; else always use hierarchy carriers
  # For now treat any value equal to 'hierarchy_carriers' as directive
  if (identical(palette, "hierarchy_carriers")) {
    base_palette <- hierarchy_palette(section = "carriers", include_groups = FALSE, include_carriers = TRUE, include_subtypes = FALSE, label_style = "id")
  } else {
    # Legacy path (kept temporarily) – will be replaced
    base_palette <- hierarchy_palette(section = "carriers", include_groups = FALSE, include_carriers = TRUE, include_subtypes = FALSE, label_style = "id")
  }
  if (is.null(sources)) sources <- names(base_palette)

  # Use matcher (drop synonym expansion—only palette + hierarchy children)
  match_df <- match_energy_sources(sources, palette_name = palette, include_full_palette = FALSE, warn = FALSE, palette_vector = base_palette)
  ...
  if (use_hierarchy && !is.null(sources) && requireNamespace("yaml", quietly = TRUE)) {
    ...
    if (!inherits(htab, "try-error")) {
      ...
      if (key == "name") {
        ...
        # If requested style yields identical labels to the breaks for all entries and a short/long alternative exists, attempt alternate style to force differentiation
        if (all(!is.na(fetched)) && all(fetched == breaks) && key %in% c("label_long","label_short","label_default")) {
          alt_key <- if (key == "label_long") "label_short" else if (key == "label_short") "label_long" else "label_short"
          if (alt_key %in% names(htab)) {
            alt_map <- htab[[alt_key]]; names(alt_map) <- htab$name
            alt_fetched <- alt_map[canonical_for_break]
            # Use alt only where it differs and is non-empty
            swap_idx <- which(!is.na(alt_fetched) & nzchar(alt_fetched) & alt_fetched != fetched)
            if (length(swap_idx)) fetched[swap_idx] <- alt_fetched[swap_idx]
          }
        }
