...
interpolate_energy_colors <- function(source1, source2, n = 10, palette = NULL) {
  ...
  if (!source1 %in% names(pal)) {
    stop("Source '", source1, "' not found in palette '", palette, "'")
  }
  if (!source2 %in% names(pal)) {
    stop("Source '", source2, "' not found in palette '", palette, "'")
  }
  color1 <- pal[source1]
  color2 <- pal[source2]
...
order_energy_sources <- function(sources, 
  ...
  if (!is.null(custom_order)) {
    ...
    # Use the order from the palette itself
    pal <- get_energy_palette(palette)
    factor_order <- names(pal)
  } else {
    # Define canonical carbon intensity order (high to low carbon)
    # For stacking: high-carbon sources should be at bottom (first factor level)
    # For legends: high-carbon sources should be listed first (traditional order)
    canonical_order <- c(
      # High carbon - Coal types
      "Coal", "Anthracite", "Bituminous Coal", "Sub-bituminous Coal", "Lignite", "Coal Power",
      
      # Medium-high carbon - Oil products  
      "Oil", "Crude Oil", "Heavy Fuel Oil", "Diesel", "Residual Fuel Oil", "Petroleum",
      
      # Medium carbon - Natural gas
      "Natural Gas", "Gas", "LNG", "Gas Turbine", "Combined Cycle Gas",
      
      # Low carbon - Nuclear
      "Nuclear", "Nuclear Power", "Nuclear PWR", "Nuclear BWR",
      
      # Very low carbon - Renewables (dispatchable first)
    ...
    if (method == "dispatchability") {
      # Reorder by dispatchability: dispatchable sources first
      dispatchable_order <- c(
        # Fully dispatchable
        "Coal", "Oil", "Natural Gas", "Nuclear", "Hydro", "Bioenergy", "Geothermal",
        "Coal Power", "Gas Turbine", "Nuclear PWR", "Hydro Turbine",
        
        # Semi-dispatchable/variable
        "Wind", "Wind Turbine", "Solar", "Solar PV",
        
        # Storage
        "Battery Storage", "Pumped Hydro"
      )
      factor_order <- dispatchable_order
    } else {
      # Default: carbon intensity order
      factor_order <- canonical_order
    }
  }
  ...
  # Apply direction parameter to control stacking order
  # direction = -1 (default): Coal at bottom of stack (traditional energy foundation)
  # direction = 1: Coal at top of stack (inverted view)
  if (!is.null(direction) && direction == 1) {
    factor_order <- rev(factor_order)
  }
  
  # Filter to only include sources that are actually present
  present_sources <- intersect(factor_order, sources)
  
  # Add any sources not in our canonical list at the end
  ...
  ordered_factor <- factor(sources, levels = present_sources, ordered = TRUE)
...
#' print(palette_order)
get_canonical_order <- function(palette = NULL, method = "carbon_intensity", reverse = FALSE) {
  # Use default palette if none specified
  if (is.null(palette)) {
    palette <- get_default_energy_palette()
  }
